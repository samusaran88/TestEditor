...
namespace RTG
{
    ...
    public static class PlaneIdHelper
    {
        ...
        {
            int index = (int)planeId;
            AxisSign result = AxisSign.Positive;
            foreach (var pi in _planeInfo[index].QuadrantInfo)
            {
                if(pi.Quadrant==planeQuadrant)
                {
                    result = pi.FirstAxisSign;
                    break;
                }
            }
            return result;
        ...
        {
            int index = (int)planeId;
            PlaneQuadrantId result = PlaneQuadrantId.First;
            foreach (var item in _planeInfo[index].QuadrantInfo)
            {
                if(item.FirstAxisSign == firstAxisSign && item.SecondAxisSign == secondAxisSign)
                {
                    result = item.Quadrant;
                    break;
                }
            }
            return result;
