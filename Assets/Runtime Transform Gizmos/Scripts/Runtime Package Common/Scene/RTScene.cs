...
namespace RTG
{
    ...
    public class RTScene : MonoSingleton<RTScene>
    {
        ...
        public void OnGameObjectWillBeDestroyed(GameObject gameObject)
        {
            if (Settings.PhysicsMode != ScenePhysicsMode.RTG) 
                return;
            if (_ignoredRootObjects.Contains(gameObject)) 
                return;
        ...
        public List<RaycastResult> GetHoveredUIElements()
        {
            ...
            raycastResults.RemoveAll(item => item.gameObject.GetComponent<RectTransform>() == null);
        ...
        public GameObjectRayHit RaycastMeshObject(Ray ray, GameObject meshObject)
        {
            ...
            if (meshCollider != null)
                raycastCollider = meshCollider;

            if (raycastCollider == null)
                raycastCollider = meshObject.GetComponent<Collider>();

            if (raycastCollider != null)
            {
                RaycastHit rayHit;
                if (raycastCollider.Raycast(ray, out rayHit, float.MaxValue))
        ...
        public GameObjectRayHit RaycastMeshObjectReverseIfFail(Ray ray, GameObject meshObject)
        {
            ...
            if (hit == null) 
                hit = RaycastMeshObject(new Ray(ray.origin, -ray.direction), meshObject);
        ...
        public void Update_SystemCall()
        {
            ...
            if (_rootGameObjectsBuffer.Capacity <= numRoots)
                _rootGameObjectsBuffer.Capacity = numRoots + 100;
            ...
            for (int rootIndex = 0; rootIndex < numRoots; ++rootIndex)
            {
                ...
                if (_ignoredRootObjects.Contains(rootObject))
                    continue;
                ...
                for (int sceneObjIndex = 0; sceneObjIndex < numObjectsInHierarchy; ++sceneObjIndex)
                {
                    ...
                    if (_ignoredRootObjects.Contains(sceneObject))
                        continue;
                    
                    if (!_sceneTree.IsObjectRegistered(sceneObject))
                    {
                        if(!_sceneTree.RegisterObject(sceneObject))
                        {
                            _ignoredRootObjects.Add(sceneObject);
                        }
                        continue;
                    }
