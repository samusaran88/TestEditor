    ...
    public class RTSceneGrid : MonoSingleton<RTSceneGrid>, IXZGrid
    {
        ...
        public void Update_SystemCall()
        {
            ...
            if (RTInputDevice.Get.Device.DidDoubleTap)
                return;
            if (!RTInputDevice.Get.Device.WasButtonPressedInCurrentFrame(0))
                return;
        ...
        public void Render_SystemCall(Camera renderCamera)
        {
            ...
            Material material2 = MaterialPool.Get.XZGrid_Plane;
            material2.SetFloat("_CamFarPlaneDist", renderCamera.farClipPlane);
            material2.SetVector("_CamWorldPos", renderCamera.transform.position);
            material2.SetMatrix("_TransformMatrix", transformMatrix);
            material2.SetFloat("_CellSizeX", Settings.CellSizeX);
            material2.SetFloat("_CellSizeZ", Settings.CellSizeZ);
            material2.SetColor("_LineColor", LookAndFeel.LineColor);
            material2.SetVector("_GridOrigin", Vector3.zero);
            material2.SetVector("_GridRight", Right);
            material2.SetVector("_GridLook", Look);
            material2.SetPass(0);
            Graphics.DrawMeshNow(MeshPool.Get.UnitQuadXZ, transformMatrix);
        }
        ...
        private void SnapToObjectHitPoint(GameObjectRayHit objectHit, SnapToPointMode snapMode)
        {
            ...
            {
                distToPlane = new Plane(Normal, Vector3.zero).GetDistanceToPoint(objectHit.HitPoint);
                YOffset = distToPlane;
                return;
            }
            ...
            distToPlane = new Plane(Normal, Vector3.zero).GetDistanceToPoint(destPt);
            YOffset = distToPlane;
        }
    }
}
