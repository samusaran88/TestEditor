...
namespace RTG
{
    ...
    public class RTUndoRedo : MonoSingleton<RTUndoRedo>
    {
        ...
        {
            return RTInput.GetKeyDown(KeyCode.Z) && RTInput.GetKey(KeyCode.LeftControl) && RTInput.GetKey(KeyCode.LeftShift);
        }

        bool GetEditorRedoHotkey()
        {
            return RTInput.GetKeyDown(KeyCode.Y) && RTInput.GetKey(KeyCode.LeftControl) && RTInput.GetKey(KeyCode.LeftShift);
        }
        ...
        private void Undo()
        {
            ...
            CanUndoRedo?.Invoke(UndoRedoOpType.Undo, answer);
            ...
            {
                UndoStart?.Invoke(action);
                action.Undo();
                UndoEnd?.Invoke(action);
            }
        }
        ...
        private void Redo()
        {
            ...
            if(_stackPointer == _actionGroupStack.Count - 1) 
                return;

            var group = _actionGroupStack[_stackPointer + 1];
            
            YesNoAnswer answer = new YesNoAnswer();
            CanUndoRedo?.Invoke(UndoRedoOpType.Redo, answer);
            ...
            foreach (var action in group.Actions)
            {
                RedoStart?.Invoke(action);
                action.Redo();
                RedoEnd?.Invoke(action);
            }
        }
        List<ActionGroup> groupsToRemove = new List<ActionGroup>();
        private void RemoveGroups(int startIndex, int count)
        {
            groupsToRemove = _actionGroupStack.GetRange(startIndex, count);
