...
namespace RTG
{
    ...
    public class MoveGizmo : GizmoBehaviour
    {
        ...
        {
            if (_isVertexSnapEnabled) 
                _postVSnapPosRestore += Gizmo.RelativeDragOffset;
        }

        private void PlaceDblSlidersInSliderPlanes(Camera camera)
        {
            if (_xySlider.IsVisible) 
                _xySlider.MakeSliderPlane(Gizmo.Transform, PlaneId.XY, _pXSlider, _pYSlider, camera);
            
            if (_yzSlider.IsVisible) 
                _yzSlider.MakeSliderPlane(Gizmo.Transform, PlaneId.YZ, _pYSlider, _pZSlider, camera);
            
            if (_zxSlider.IsVisible) 
                _zxSlider.MakeSliderPlane(Gizmo.Transform, PlaneId.ZX, _pZSlider, _pXSlider, camera);
        }
