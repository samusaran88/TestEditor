...
namespace RTG
{
    ...
    public class GizmoLineSlider3D : GizmoSlider
    {
        ...
        {
            foreach(var s in _scalerHandles)
            {
        ...
        public bool IsScalerHandleRegistered(int handleId, int scaleDragAxisIndex)
        {
            foreach (var s in _scalerHandles)
            {
                if (s.HandleId == handleId)
                {
                    return s.ContainsScaleDragAxisIndex(scaleDragAxisIndex);
                }
            }
            return false;
        }
        ...
        public Vector3 GetRealDirection()
        {
            ...
            if (_scaleDrag.IsActive) 
                sign = Mathf.Sign(TotalDragScale);
            else 
        ...
        public float GetRealLength(float zoomFactor)
        {
            ...
            if (_scaleDrag.IsActive)
            {
                length *= _scaleDrag.TotalScale;
            }
            else if (Gizmo.IsDragged && IsScalerHandleRegistered(Gizmo.DragHandleId, ScaleDragAxisIndex))
            {
                length *= Gizmo.TotalDragScale[ScaleDragAxisIndex];
            }
